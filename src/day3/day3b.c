...
bool isPrecedingInOccurrence(char *first, char *second) {
    return first < second;
}

// TODO: Spør gpt om enabed inne der vs ute
bool processLine(char *line, void *arg) {
    ...
    while ((nextMul = strstr(currentPosition, "mul("))) {
        nextToggle = strstr(currentPosition, enabled ? "don't()" : "do()");
        if (nextToggle && isPrecedingInOccurrence(nextToggle, nextMul)) {
            enabled = !enabled;
            currentPosition = nextToggle;
            continue;
        }
        currentPosition = nextMul + 1;
        if (!enabled) {
            continue;
        }
